...
def dev_focus_tasks(task, headers, taiga_url, from_date, to_date, dev_focus_data):
    task_history_url = f"{taiga_url}/history/task/{task['id']}"
    try:
        response = requests.get(task_history_url, headers=headers)
        response.raise_for_status()
        history_data = response.json()
        in_progress_date = extract_new_to_in_progress_date(history_data)
        print(in_progress_date == None)

        if in_progress_date:
            in_progress_date = datetime.fromisoformat(str(in_progress_date)[:-6])

            if in_progress_date > datetime.fromisoformat(from_date) and in_progress_date < datetime.fromisoformat(
                    to_date):
...
def get_all_dev_focus(project_id, from_date, to_date, threshold, members, auth_token):
    tasks = get_dev_focus(project_id, from_date, to_date, threshold, members, auth_token)
    taiga_url = os.getenv('TAIGA_URL')
...
def get_dev_focus(project_id, from_date, to_date, threshold, members, auth_token):
    all_tasks = []
    for member in members:
        tasks = task_by_member_in_date_range(project_id, member, from_date, to_date, auth_token)
...
def member_map(project_id, from_date, to_date, member, auth_token, member_tasks):
    tasks = task_by_member_in_date_range(project_id, member, from_date, to_date, auth_token)
...
def sorted_tasks_data(task, headers, taiga_url, from_date, to_date, member_tasks):
    task_history_url = f"{taiga_url}/history/task/{task['id']}"
    try:
        response = requests.get(task_history_url, headers=headers)
        response.raise_for_status()
        history_data = response.json()
        in_progress_date, closed_date = extract_state_change_dates(history_data)
        if (in_progress_date is not None or closed_date is not None):
            in_progress_date = datetime.fromisoformat(str(in_progress_date)[:-6]) if in_progress_date is not None else None
            closed_date = datetime.fromisoformat(str(closed_date)[:-6]) if closed_date is not None else None
            if ((in_progress_date is not None and datetime.fromisoformat(from_date) < in_progress_date < datetime.fromisoformat(
                    to_date))) or ( closed_date is not None and datetime.fromisoformat(from_date) < closed_date < datetime.fromisoformat(
                to_date)):
                task_data = {
                    "taskId": task["id"],
                    "taskDesc": task["subject"],
