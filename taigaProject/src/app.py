...
from taigaProject.src.taigaApi.userStory.get_user_story_history import (
    get_user_story
)
from taigaProject.src.taigaApi.task.get_task_history import (
    get_task_history
)
...
def get_open_user_stories(project_id, auth_token):
    ...
    open_user_stories = [
        story for story in user_stories if not story["is_closed"]
    ]
...
def get_closed_tasks_per_week(project_id, auth_token):
    ...
    for task in closed_tasks:
        ...
        existing_week = next(
            (week for week in task_groups if week["weekEnding"] == week_end),
            None
        )
